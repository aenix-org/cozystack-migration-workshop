...
Фаза 2, машина-конвертер (`conversion-VM` в именах файлов):
...
пароль `ubuntu`), впишите креды в `convert.sh` и запустите. `virt-v2v` переупакует
OVA в qcow2 и зальёт в ваш бакет, а в конце напечатает presigned-ссылку.

Фаза 3, ваша виртуальная машина (`app-VM` в именах файлов). Вставьте presigned-ссылку в `03-app-vm.yaml` (поле `url`) и
примените:
...
перезапустить), потом `connect-managed.sh` переписывает настройки приложения на адреса управляемых сервисов,
