...
|---|---|---|---|
| Документ | Одна запись. Набор полей, вложенных объектов и списков | Строка таблицы | У соседней записи набор полей может быть другим, и это не ошибка |
| Коллекция | Набор документов | Таблица | Схемы у неё нет по умолчанию. Схему можно добавить, но это отдельное решение |
| BSON | Двоичный формат, в котором лежат документы | — | Похож на JSON, но с типами: дата — это дата, а не строка |
| `_id` | Уникальный ключ документа | Первичный ключ | Создаётся сам, если вы его не задали |
| Набор реплик | Несколько копий, одна главная, остальные подхватывают | Кластер с HA | Копии голосуют между собой, поэтому их нужно нечётное число |
| Шардирование | Разделение данных между наборами реплик | — | Про объём, а не про надёжность |
| `mongosh` | Командная оболочка MongoDB | PowerCLI | Это полноценный JavaScript, а не язык запросов |
| Точечная запись поля | Обращение к вложенному полю через точку: `car.plate` | Путь к файлу в папке | Работает и внутрь списков: `members.age` |
...
| Валидатор схемы | Правило, которому обязан соответствовать документ | Проверка формы перед сохранением | Включается вручную и задним числом, схемы по умолчанию нет |
| `$lookup` | Способ подтянуть данные из другой коллекции | JOIN | Работает, но это не JOIN: он односторонний и заметно дороже |
...
```bash
kubectl --kubeconfig="$HOME/tenant.kubeconfig" apply -f mongodb.yaml
```
...
важное отличие от «просто JSON»: по дате можно сравнивать и сортировать, по строке
...
такой индекс потеряет документы без поля — они в индексе просто отсутствуют. MongoDB в
...
доступным не везде и не всегда очевидным.
...
**Остановитесь. Посмотрите на результат и подумайте, прежде чем читать дальше.**

Сколько пропусков вернулось? Где пятый? Что произошло бы при такой же ошибке в
реляционной базе — и почему это лучше?
...
`guest` — оно пропустит: документ по-прежнему законен, просто с лишним полем. Запретить
...
| Изменение схемы | миграция и релиз | новое поле просто появляется |
...
Три виртуальные машины под набор реплик — три заявки на ресурсы, установка, настройка
голосования между копиями, мониторинг. Плюс отдельный разговор с ИБ про то, что это за
новая база и кто её будет обновлять. Недели, и это только чтобы начать.
