...
| Чарт (chart) | Пакет Helm: шаблоны, значения по умолчанию, схема | OVF-шаблон | Разворачивается многократно с разными параметрами в одном месте |
| Релиз (release) | Конкретное разворачивание чарта под своим именем | Развёрнутая из шаблона VM | Помнит свою историю версий и умеет откатываться |
| values | Параметры, с которыми разворачивают чарт | Поля мастера развёртывания OVF | Обычный YAML, лежит в Git рядом с остальным |
| values.schema.json | Описание допустимых значений | Валидация полей в мастере | Проверяет до применения, а не в процессе |
| ApplicationDefinition | Запись в каталоге платформы: что показать и что развернуть | Пункт в Content Library | Один на кластер и виден всем тенантам |
| Cluster-scoped | Объект без namespace, общий для всего кластера | Настройка уровня vCenter | Права на него у платформенной команды, не у тенанта |
...
Внешние каталоги обычно приезжают путём `GitRepository` → `HelmChart`: админ добавляет
...
Смысл разбирать всё это руками, как мы сейчас, никуда при этом не девается. Сгенерированный
скелет придётся читать и править, а править то, чего не понимаешь, — худший из известных
способов работы.
