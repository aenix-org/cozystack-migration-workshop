...
|---|---|---|
| `~/tenant.kubeconfig` | Управляющий кластер Cozystack, ваш тенант | смотрим на Redis: адрес, состояние |
...
Из этого свойства, кстати, вырастет предсказуемая неудача на шаге 7. Приложение,
которое молча продолжает работать, — приятно в бою и коварно при отладке.
...
**Урок шире этой ошибки.** Отказ, который не мешает работать, — самый дорогой вид отказа.
