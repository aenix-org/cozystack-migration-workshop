...
|---|---|---|---|
| Job | Задача: запустить и дождаться, пока успешно завершится | Запланированная задача в гостевой ОС | Job сам создаёт себе машину под запуск и сам её убирает |
| Deployment | Описание вечно работающего приложения | vApp | Никогда не «завершается успешно» — исчезнувшую копию создают заново |
| Объектное хранилище | Хранилище, где нет файлов и каталогов, есть ключ и содержимое | Датастор | Не монтируется. Кладут и забирают целиком, по HTTP |
| Бакет | Именованная область в объектном хранилище | Папка на датасторе | Не вложенная: бакет — верхний уровень, «папки» внутри него часть имени объекта |
| S3 | Протокол доступа к объектному хранилищу поверх HTTP | — | Прямого аналога нет; ближе всего к REST API, чем к NFS |
| Ключи доступа | Пара «access key / secret key» вместо логина и пароля | Сервисная учётка | Ключи выдаются на бакет, а не на человека |
| Secret | Объект кластера, куда кладут пароли и ключи | Учётка в credential store | Внутри кластера это base64, а не шифрование. Прячет от глаз, не от админа |
| emptyDir | Временный диск, живущий ровно столько, сколько под | Временный vmdk | Исчезает вместе с подом, без вариантов восстановления |
...
⚠️ **Через `kubectl` этот секрет не прочитать, и это не поломка.** Как и кубконфиг в лабе
0, реквизиты выдаёт только дашборд.
...
**Остановитесь и подумайте, прежде чем читать дальше.** Что именно не сошлось? Вспомните,
какой узел вы заказали в лабе 0 и сколько памяти запросил Job.
