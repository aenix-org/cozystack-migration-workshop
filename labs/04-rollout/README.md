...
|---|---|---|---|
| RollingUpdate | Замена копий по очереди, а не всех разом | Обновление ВМ по одной руками | Кластер делает это сам и останавливается, если новая копия не поднялась |
| Ревизия | Сохранённый снимок описания приложения | Снапшот ВМ | Хранит только описание, никаких данных внутри нет |
...
| maxUnavailable | Сколько копий разрешено погасить, не дождавшись замены | Сколько ВМ гасим за раз | Округляется вниз, поэтому при трёх копиях выходит ноль |
| readinessProbe | Проверка «готов принимать трафик» | Health check в пуле балансировщика | Она же тормозит выкатку, а не только выводит из балансировки |
...
ключу, и у `volumes` таким ключом является `name`:
...
Механизм понятен. Выкатим ещё одну версию — третью:
...
⚠️ **Остановитесь и подумайте, прежде чем читать дальше.** Вопросов здесь два, и второй
важнее первого. Первый: почему новая копия не запускается? Второй: что сейчас происходит с
сервисом — он лежит?
...
Отсюда практическое следствие для дежурного: **застрявшая выкатка не является инцидентом.**
