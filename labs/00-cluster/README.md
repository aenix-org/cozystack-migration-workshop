...
```bash
# HelmRelease — внутренний объект платформы: через него она устанавливает ваш заказ.
# По нему видно, дошла ли до вас очередь. Имя складывается из типа и имени заказа.
...
```bash
# annotate вешает на объект пометку-аннотацию. Эта пометка — сигнал платформе
# «займись мной сейчас».
#   date +%s      текущее время в секундах: значение должно отличаться от прошлого,
#                 иначе сигнал не считается новым и ничего не произойдёт
#   --overwrite   разрешить перезаписать пометку, если она уже стоит
