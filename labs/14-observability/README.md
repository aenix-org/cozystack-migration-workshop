# Лаба 14 · Найти свой всплеск в графиках
...
|---|---|---|---|
| Метрика | Число, снимаемое регулярно: сколько процессора, памяти, запросов | Счётчик в графиках vCenter | Хранится историей, а не последним значением |
| Метка (label) | Пара «имя=значение» на метрике: под, namespace, кластер | Объект, к которому привязан счётчик | Меток много, и по любой из них можно резать и группировать |
| Временной ряд | Одна метрика с одним набором меток, в динамике | Один график в vCenter | Каждое сочетание меток — отдельный ряд, их тысячи |
| Скрейп (scrape) | Сбор: агент раз в N секунд опрашивает источники | Сбор счётчиков vCenter | Тянет агент, а не отдаёт приложение |
| PromQL | Язык запросов к метрикам | — | Прямого аналога нет: в vCenter вы выбираете счётчик, здесь пишете выражение |
| VictoriaMetrics | Хранилище метрик под капотом | База статистики vCenter | Совместима с Prometheus, но не он |
| Grafana | Интерфейс к метрикам и логам | Вкладка Performance | Отдельный продукт, дашборды пишутся руками или берутся готовыми |
| Retention | Сколько хранить историю | Уровни статистики в vCenter | Задаётся параметром, по умолчанию 3 дня и 14 дней двумя хранилищами |
| Логи | Строки, которые пишет приложение | Логи в гостевой ОС | Собираются централизованно, свой язык запросов, не PromQL |
...
| Пример | «в 16:41:30 под съел 240 миллиядра» | «16:41:31 ERROR connection refused» |
...
выбрасывать ненужные метки, отключать сбор редких метрик. Это нормальная эксплуатационная
...
**Если вы видите список подов** с `vmagent` и `fluent-bit` — всё на месте, идите дальше.

**Если namespace не существует:**

```
Error from server (NotFound): namespaces "cozy-monitoring" not found
```
...
```bash
export KUBECONFIG=~/lab.kubeconfig
kubectl apply -f ../03-scale/hpa.yaml
kubectl apply -f ../03-scale/fortio.yaml
kubectl rollout status deployment/fortio
```

Дождитесь, пока в `kubectl get hpa rickroll` вместо `<unknown>` появится процент — это
занимает пару минут. Затем пробросьте порт генератора и дайте ту же нагрузку, что в лабе 3,
иначе ступенька автомасштабирования не появится:
...
⚠️ Чтобы не попасть в эту ситуацию, включайте `Monitoring agents` сразу в лабе 0 — там
про это сказано в шаге 1.

<details>
<summary><b>Что именно там работает и куда всё уезжает</b></summary>

В namespace `cozy-monitoring` вашего кластера живут:
...
| `metrics-server` | **не часть мониторинга**: ставится вместе с кластером и кормит `kubectl top` и автомасштабирование. Просто живёт по соседству |

Обратите внимание: **хранилища здесь нет**. Всё собранное уезжает в тенант, в общее
хранилище метрик рядом с Grafana. Так сделано намеренно: кластер `lab` — вещь
одноразовая, вы его удалите, а записи о том, как он себя вёл, должны пережить удаление.

Посмотреть, куда именно уезжает:
...
На воркшопе ведущий назовёт точный адрес. Найти его самому можно в дашборде: тенант →
приложение `Monitoring` → вкладка с ingress.

⚠️ **Если приложения `Monitoring` в вашем тенанте нет** — своей Grafana у вас тоже нет, и
метрики уезжают в мониторинг родительского тенанта. Тогда либо разверните `Monitoring`
из каталога (раздел Administration), либо спросите адрес общей Grafana у ведущего.
...
Начнём с самого грубого вопроса: какие вообще поды видны в кластере.

```promql
count by (pod) (
  container_cpu_usage_seconds_total{cluster="kubernetes-lab", namespace="default"}
)
```

Переключите вид с Graph на **Table** — так список читается лучше. Вы должны увидеть
`rickroll-...`, `fortio-...` и, если делали лабу 11, `propusk-build-...`.
...
managed-сервисов. Без фильтра вы получите кашу из всего тенанта.
...
Теперь превратим счётчик в понятную величину:
...
Всплеск нашли. Теперь посмотрим, что на него ответил кластер.

```promql
kube_horizontalpodautoscaler_status_current_replicas{
  cluster="kubernetes-lab", horizontalpodautoscaler="rickroll"
}
```
...
Наложите её на график из шага 4 (в Explore можно добавить второй запрос кнопкой `+ Add
query`). Видно, что ступенька идёт **за** всплеском с отставанием в несколько десятков
секунд: сначала процессор вырос, потом автомасштабирование это заметило и среагировало.
Это и есть ответ на вопрос «почему пользователи всё-таки успели заметить замедление».
...
объекта HPA, `deployment` — имя Deployment, и так далее по всем типам.
...
Первая — сколько копий создано, вторая — сколько из них реально готовы принимать трафик.
...
Посмотрим ровно на эту величину:
...
sum by (pod) (
  kube_pod_container_resource_requests{
    cluster="kubernetes-lab", namespace="default",
    pod=~"rickroll-.*", resource="cpu"
  }
)
```

Единица на этом графике означает «под потребляет ровно столько, сколько запросил».
...
**Кто в кластере ест больше всех процессора, топ-10:**
...
выглядит Job: в отличие от приложения, у его линии есть конец.

## Шаг 8. Заглядываем в логи

Переключите источник данных на **`vlogs-generic`** и выполните:

```logsql
kubernetes_namespace_name:default AND kubernetes_pod_name:rickroll*
```

Это уже не PromQL — у логов свой язык. Синтаксис отличается принципиально: там вы
описывали числовые ряды, здесь — отбираете строки.

Сопоставьте время: возьмите минуту всплеска из шага 4 и посмотрите логи за неё. У nginx
в эту минуту будет всплеск записей о запросах.
...
export COZY_KUBECONFIG=~/.kube/workshop     # тоже необязательно
./check.sh
```
...
| Глубина, когда разобрался | ограничена набором счётчиков | ограничена вашей фантазией |
