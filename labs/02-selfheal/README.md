...
**Что вы должны увидеть** — три строки примерно такого вида:
...
Четыре секунды. Разброс на учебном стенде — от двух до десяти, в зависимости от того, чем
...
⚠️ **Не выходите из этого окна.** Вывод идёт построчно и виден, только пока вы к нему
подключены: отсоединитесь — и `kubectl logs` покажет пустоту, потому что строки уходят в
буфер. Нужен второй терминал, а не фоновый запуск.
...
Пять секунд сервис отвечал ошибкой. Копия вернулась быстро, но пока её не было — отвечать
было некому.
