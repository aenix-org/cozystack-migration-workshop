...
|---|---|---|
| **OLAP** | Нагрузка «мало запросов, но каждый читает миллионы строк» | **Отчёт из vRealize за квартал**, но — |
...
| Термин | Что это | Похоже на… но |
|---|---|---|
| **OLTP** | Нагрузка «много мелких операций»: создать заявку, поменять статус | **Работа vCenter со своей базой**, но — |
...
| `02-generate.sql` | Генерация нескольких миллионов строк, чтобы было что считать | выполняете следом |
