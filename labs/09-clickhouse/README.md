...
| Replicas | **1** | учебный стенд. В проде минимум две: потеря узла не должна означать потерю отчётов |
...
| Log storage size | `2Gi` | под журнал запросов, мы в него заглянем |
...
```bash
kubectl --kubeconfig="$HOME/tenant.kubeconfig" apply -f clickhouse.yaml
```

Кубконфиг тенанта берётся в дашборде: **Info → вкладка Secrets →
`kubeconfig-tenant-workshopXX`**.
...
**Почему адрес и пароль переменными пода, а не прямо в команде.** Всё, что вы напишете в
`kubectl exec`, попадёт в историю вашей оболочки и в список процессов на узле. Переменные
пода задаются один раз, и дальше пароль в командах не появляется. Привычка полезная; в
лабе 8 про это отдельный разговор.
...
```powershell
function ch { $input | kubectl exec -i ch-workbench -- sh -c 'curl -sS -u "$CH_AUTH" --data-binary @- "$CH_URL?default_format=PrettyCompact"' }
"SELECT version()" | ch
```
...
**Третье, неочевидное: сжатие ускоряет, а не замедляет.** Кажется, что распаковка — это
лишняя работа. На практике узкое место — диск, а не процессор: прочитать 35 килобайт и
...
Посмотрите на `bytes_read` в конце вывода и сравните с общим размером таблицы:

```bash
ch <<'SQL'
SELECT formatReadableSize(sum(bytes_on_disk)) AS total
FROM system.parts
WHERE table = 'passes' AND active
SQL
```

Запрос прошёл по миллиону строк, прочитав считанные проценты таблицы: `duration_min`
...
Вся история ваших запросов с длительностью и объёмом прочитанного. Это та самая таблица,
...
**Остановитесь. Подумайте, прежде чем читать дальше.**

Сколько строк база прочитала, чтобы вернуть одну? Сколько прочитал бы PostgreSQL с
индексом по `pass_id`? И почему разница именно такая?
...
**Урок шире этой ошибки.** ClickHouse не «быстрая база». Он база, устроенная под один тип
...
И второе, менее очевидное. Управляемый сервис — это чужие умолчания. Версия ClickHouse,
