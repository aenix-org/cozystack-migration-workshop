...
|---|---|---|
| `~/tenant.kubeconfig` | Управляющий кластер Cozystack, ваш тенант | смотрим на managed-сервисы: Harbor, базы, очереди |
...
⚠️ **Если Harbor висит в состоянии «не готов» дольше пятнадцати минут** — вероятнее всего,
в тенанте нет объектного хранилища. Harbor хранит слои образов только в S3-совместимом
хранилище, и бакет под них создаётся автоматически, но лишь если хранилище в тенанте
включено. Это настройка тенанта, а не ваша: напишите в чат воркшопа, её включат сверху.
...
⚠️ **Через `kubectl` этот секрет вам, скорее всего, не покажут, и это не поломка.** Роль
в тенанте не даёт читать секреты командой; реквизиты выдаёт дашборд, и только он. Сделано
так намеренно: пароли не должны утекать в историю команд и в логи.
...
<details>
<summary><b>Что произошло</b></summary>
...
**Урок шире этой ошибки.** `ImagePullBackOff` — это почти всегда одно из трёх: опечатка в
...
export COZY_KUBECONFIG=~/tenant.kubeconfig
./check.sh
```

Первые две переменные обязательны. Третья — путь к тенантному кубконфигу; если её не
задать, скрипт поищет файл `~/tenant.kubeconfig`, а не найдя — пропустит проверки на
