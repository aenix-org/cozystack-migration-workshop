...
у вас что-то иначе: просто сборщик метрик в вашем кластере уже проработал какое-то время и
успел опросить поды. `<unknown>` появляется на кластере, поднятом только что. Если у вас
